...
- **Primary-source quotes do work.** The best quotes are short, in context, and immediately reframed in the writer's own voice — not block-quoted as decoration. Pull them from `resources/quotes/quotes.md` (the exported Stoa quote library) and cite the `Quote ID` so they link into the app. Across a multi-lesson piece, vary the openings — many quotes in the library begin with "It is...", "We...", or "When...", and stacking those reads as monotone. Favor short quotes (under 5 sentences); longer passages overflow the in-app quote card, so trim to the load-bearing sentences and put the rest in the prose around the quote.
